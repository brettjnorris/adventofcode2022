...
impl Backpack {
    ...
    fn priority(&self) -> Option<u32> {
        ...
        Some(dupes.iter().map(|char| score_item(&char).unwrap() as u32).sum())
    }

    fn find_duplicates(&self) -> Vec<char> {
        let first = &self.compartments[0];
        let second = &self.compartments[1];
        first.intersection(second).map(|i| *i).collect::<Vec<char>>()
    }
}
...
fn find_duplicates(backpacks: Vec<Backpack>) -> Vec<char> {
    let dupes = backpacks
        .iter()
        .skip(1)
        .fold(backpacks[0].items.clone(), |acc: HashSet<char>, backpack| {
            acc.intersection(&backpack.items.clone()).cloned().collect::<HashSet<char>>()
        });
...
pub fn part_one(input: &str) -> Option<u32> {
    input
        .lines()
        .map(|line| {
            Backpack::from_string(line).priority()
        })
        .sum()
}
